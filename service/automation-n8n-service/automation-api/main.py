...
def log_event(event: str, **kwargs):
    payload = {"ts": datetime.now(UTC).isoformat(), "event": event}
    if kwargs:
...
def ai_linkedin(payload: LinkedInGenIn, request: Request):
    ...
    log_event(
        ...
        req_id=req_id,
        duration_ms=dt_ms,
        response_size=len(json.dumps(out)),
    )
    return out
...
def ai_twitter(payload: TwitterGenIn, request: Request):
    ...
    dt_ms = int((time.perf_counter() - t0) * 1000)
    log_event(
        "ai_twitter.success", req_id=req_id, duration_ms=dt_ms, tweets=len(tweets)
    )
    return out
...
async def ws_endpoint(ws: WebSocket):
    ...
    try:
        while True:
            ...
            log_event(
                "ws.sent", session_id=session_id, echo_bytes=len(data.encode("utf-8"))
            )
    except Exception as e:
