#!/usr/bin/env python3
"""Stop script for Logs Pipeline - terminates workflow, stops containers, and cleans up volumes."""
...
CONTAINER_NAMES = ["loki-development", "promtail-development", "grafana-development"]
VOLUME_NAMES = [
    "loki-data",
    "loki-config",
    "promtail-config",
...
async def main():
    if len(sys.argv) < 2:
        logger.error("Usage: python3 logs_pipeline_stop.py <workflow_id>")
        sys.exit(1)

    workflow_id = sys.argv[1]

    logger.info("=" * 80)
    logger.info("Stopping Logs Pipeline (Grafana + Loki + Promtail)...")
    ...
    if workflow_stopped and containers_stopped and volumes_removed:
        logger.info("✅ Logs Pipeline stopped and cleaned up successfully!")
    else:
        logger.warning("⚠️ Logs Pipeline stopped with some warnings. Check logs above.")
    logger.info("=" * 80)
