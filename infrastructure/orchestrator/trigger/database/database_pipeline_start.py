#!/usr/bin/env python3
"""
Simple trigger script to start Database Pipeline using Temporal workflows.
"""
...
class WorkflowConfig:
    DEFAULT_SERVICE_NAME = "database-pipeline"
    DEFAULT_WORKFLOW_NAME = "DatabasePipelineWorkflow"
    DEFAULT_TASK_QUEUE = "database-pipeline-queue"
...
async def start_database_pipeline(config: WorkflowConfig) -> str | None:
    try:
        ...
        logger.info("Database Pipeline workflow started successfully!")
...
async def main():
    logger.info("Starting Database Pipeline (Neo4j + Qdrant)...")
    logger.info("=" * 80)
    ...
    workflow_id = await start_database_pipeline(config)
    ...
    if workflow_id:
        ...
        logger.info("   Neo4j Browser: http://localhost:7474 (neo4j/Neo4jPassword123!)")
        logger.info("   Neo4j Bolt:    bolt://localhost:7687")
        logger.info("   Qdrant REST:   http://localhost:6333")
        logger.info("   Qdrant gRPC:   localhost:6334")
        logger.info("   Qdrant Dashboard: http://localhost:6333/dashboard")
        logger.info("")
        logger.info("🛑 To stop this workflow:")
        logger.info(f"   python3 trigger/database_pipeline/stop.py {workflow_id}")
        logger.info("=" * 80)
    else:
        logger.error("=" * 80)
        logger.error("❌ Failed to start Database Pipeline workflow")
        sys.exit(1)
