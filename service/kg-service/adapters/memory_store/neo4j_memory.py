...
class Neo4jMemoryStore(MemoryStorePort):
    ...
    async def retrieve_memories(
        ...
        async with self.driver.session() as session:
            ...
            """
            result = await session.run(
                query, memory_type=memory_type.value, limit=limit
            )
    ...
    async def update_memory_access(self, memory_id: str) -> bool:
        async with self.driver.session() as session:
            ...
            """
            result = await session.run(
                query, memory_id=memory_id, now=datetime.utcnow().isoformat()
            )
    ...
    async def decay_memories(self) -> int:
        async with self.driver.session() as session:
            decay_date = (
                datetime.utcnow() - timedelta(days=self.decay_days)
            ).isoformat()
            query = """
            ...
            """
            result = await session.run(
                query, long_term=MemoryType.LONG_TERM.value, decay_date=decay_date
            )
            record = await result.single()
