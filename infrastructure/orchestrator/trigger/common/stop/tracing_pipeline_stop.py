#!/usr/bin/env python3
"""Stop script for Tracing Pipeline - terminates workflow, stops containers, and cleans up volumes."""
...
CONTAINER_NAMES = ["jaeger-development", "grafana-development"]
VOLUME_NAMES = [
    "jaeger-data",
    "grafana-data",
]
...
async def main():
    if len(sys.argv) < 2:
        logger.error("Usage: python3 tracing_pipeline_stop.py <workflow_id>")
        sys.exit(1)

    workflow_id = sys.argv[1]

    logger.info("=" * 80)
    logger.info("Stopping Tracing Pipeline (Grafana + Jaeger)...")
    ...
    if workflow_stopped and containers_stopped and volumes_removed:
        logger.info("✅ Tracing Pipeline stopped and cleaned up successfully!")
    else:
        logger.warning(
            "⚠️ Tracing Pipeline stopped with some warnings. Check logs above."
        )
    logger.info("=" * 80)
