...
class SpacyEntityExtractor(EntityExtractorPort):
    def __init__(
        self, model_name: str = "en_core_web_sm", entity_types: list[str] = None
    ):
        try:
            ...
            subprocess.run(
                ["python", "-m", "spacy", "download", model_name], check=False
            )
            self.nlp = spacy.load(model_name)
        ...
        for ent in doc.ents:
            if ent.label_ in self.entity_types:
                entity = Entity(
                    ...
                    confidence=0.8,
                    properties={"start_char": ent.start_char, "end_char": ent.end_char},
                )
