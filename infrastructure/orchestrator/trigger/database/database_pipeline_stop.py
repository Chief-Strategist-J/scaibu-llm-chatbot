#!/usr/bin/env python3
"""Stop script for Database Pipeline - terminates workflow, stops containers, and cleans up volumes."""
...
CONTAINER_NAMES = ["neo4j-development", "qdrant-development"]
VOLUME_NAMES = [
    "neo4j-data",
    "neo4j-logs",
    "neo4j-import",
...
async def main():
    if len(sys.argv) < 2:
        logger.error("Usage: python3 database_pipeline_stop.py <workflow_id>")
        sys.exit(1)

    workflow_id = sys.argv[1]

    logger.info("=" * 80)
    logger.info("Stopping Database Pipeline (Neo4j + Qdrant)...")
    ...
    if workflow_stopped and containers_stopped and volumes_removed:
        logger.info("✅ Database Pipeline stopped and cleaned up successfully!")
    else:
        logger.warning(
            "⚠️ Database Pipeline stopped with some warnings. Check logs above."
        )
    logger.info("=" * 80)
