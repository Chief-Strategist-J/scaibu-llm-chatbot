#!/usr/bin/env python3
"""
Simple trigger script to start Metrics Pipeline using Temporal workflows.
"""
...
class WorkflowConfig:
    DEFAULT_SERVICE_NAME = "metrics-pipeline"
    DEFAULT_WORKFLOW_NAME = "MetricsPipelineWorkflow"
    DEFAULT_TASK_QUEUE = "metrics-pipeline-queue"
...
async def start_metrics_pipeline(config: WorkflowConfig) -> str | None:
    try:
        ...
        logger.info("Metrics Pipeline workflow started successfully!")
...
async def main():
    logger.info("Starting Metrics Pipeline (Grafana + Prometheus)...")
    logger.info("=" * 80)
    ...
    workflow_id = await start_metrics_pipeline(config)
    ...
    if workflow_id:
        ...
        logger.info(f"   python3 trigger/metrics_pipeline/stop.py {workflow_id}")
        logger.info("=" * 80)
    else:
        logger.error("=" * 80)
        logger.error("❌ Failed to start Metrics Pipeline workflow")
        sys.exit(1)
